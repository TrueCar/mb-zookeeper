...
module ZooKeeper
  class Connection < JZooKeeper
    ...
    def initialize(host, args = {})
      ...
      @watcher.extend Extensions::Watcher
    ...
    def create(path, data = "", args = {})
      ...
      java_acls = acls.collect{|acl| Extensions::ACL.to_java(acl)}
      java_mode = Extensions::CreateMode.to_java(mode)

      if callback
        callback.extend Extensions::AsyncCallback::StringCallback unless callback.is_a?(Proc) || callback.respond_to?(:processResult)
    ...
    def get(path, args = {}, &block)
      ...
      stat     = Extensions::Stat.new

      if callback
        callback.extend Extensions::AsyncCallback::DataCallback unless callback.is_a?(Proc) || callback.respond_to?(:processResult)
    ...
    def exists?(path, opts = {})
      ...
      if callback
        callback.extend Extensions::AsyncCallback::StatCallback unless callback.is_a?(Proc) || callback.respond_to?(:processResult)
    ...
    def delete(path, args = {})
      ...
      if callback
        callback.extend Extensions::AsyncCallback::VoidCallback unless callback.is_a?(Proc) || callback.respond_to?(:processResult)
    ...
    def children(path, args = {})
      ...
      if callback
        callback.extend Extensions::AsyncCallback::ChildrenCallback unless callback.is_a?(Proc) || callback.respond_to?(:processResult)
    ...
    def acls(path, args = {})
      ...
      if callback
        callback.extend Extensions::AsyncCallback::AclCallback unless callback.is_a?(Proc) || callback.respond_to?(:processResult)
    ...
    def set_acl(path, args = {})
      ...
      if callback
        ...
        super(path, acls.collect{|acl| Extensions::ACL.to_java(acl)}, version, callback, context)
      else
        super(path, acls.collect{|acl| Extensions::ACL.to_java(acl)}, version)
      end
    rescue Exception => e
      raise_keeper_exception(e, caller)
    end
  ...
  private
    def raise_keeper_exception(e, orig_caller)
      if e.respond_to?(:code) and Exceptions::KeeperException.recognized_code?(e.code)
        raise Exceptions::KeeperException.by_code(e.code), nil, orig_caller
      else
        raise e
      end
    end
